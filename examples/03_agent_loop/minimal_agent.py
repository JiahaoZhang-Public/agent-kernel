#!/usr/bin/env python3
"""Minimal AgentLoop example: one tool, one prompt.

The simplest possible agent — a single read_file tool and a prompt
that asks the LLM to read a file and report its contents.
...
# ── Tool definition (metadata-only, no execution logic) ─────────────
read_file = ToolDef(
    name="read_file",
...
async def main() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Seed the workspace with a file for the agent to read
        hello_file = workspace / "hello.txt"
        hello_file.write_text("Hello from Agent OS Kernel! This file was read by an AI agent.")
